...
def merge_images(images_paths: List[str], save_dir: str, save_file: str) -> None:
    ...
    try:
        ...
        pass
    new_im.save(f'{save_dir}{os.sep}{save_file}')


def draw_graph(graph, t, d, file, dr, r_n=None, r_e=None, c=None, w=True, f='png', clean=True, draw_id=True, map_style=False, allowed_layers=None):
    if type(graph) == list:
        graph = {"Locations": graph}
    gv = GraphVisualizer()
    try:
        gv.visualise(graph, title=t, description=d, world=w, emph_nodes_ids=r_n, emph_edges=r_e, comments=c, draw_id=draw_id, map_style=map_style, allowed_layers=allowed_layers).render(
...
class GraphVisualizer:
    ...
    def _visualise_process(self, json_dict_or_list, parent_key: str, parent_name: str, graph: BaseGraph,
                           emph_nodes_ids: list = None, comments: dict = None, world=False, draw_id = True, map_style=False, allowed_layers: list = None) -> list:
        """
        ...
        if not allowed_layers:
             allowed_layers = ["Characters", "Items", "Narration"]
        ...
        if isinstance(json_dict_or_list, dict):
            ...
            for k, v in json_dict_or_list.items():
                if k in "LSide":
                    current_list.extend(self._visualise_process(v, 'root', 'root', graph, emph_nodes_ids, comments, world, draw_id, map_style, allowed_layers))
                elif k in ("Id", "Name", "Attributes", "Connections", "Preconditions", "Instructions"):
                    current_dict[k] = v
                elif k in "Locations":
                    current_list.extend(self._visualise_process(v, k, 'root', graph, emph_nodes_ids, comments, world, draw_id, map_style, allowed_layers))
                elif k in allowed_layers:
                    current_list.extend(self._visualise_process(v, k, parent_name, graph, emph_nodes_ids, comments, world, draw_id, map_style, allowed_layers))
            ...
            for i in range(len(lst)):
                ...
                current_list.extend(self._visualise_process(lst[i], parent_key, nr, graph, emph_nodes_ids, comments, world, draw_id, map_style, allowed_layers))
        return current_list

    def visualise(self, what: Union[list, dict], title: str = None, description: str = None,
                  world: bool = False, emph_nodes_ids: list = None, emph_edges: list = None, comments: dict = None, draw_id = True, map_style=False, allowed_layers: list=None) -> BaseGraph:
        """
        ...
        self._vertex_counter = 1
        node_list = self._visualise_process(what, 'root', '', graph, emph_nodes_ids, comments, world=world, draw_id = draw_id, map_style=map_style, allowed_layers=allowed_layers) # sprawdzić, czy root czy Root
