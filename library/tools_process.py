...
def apply_instructions_to_world(production: dict, variant: list, world: Union[list, dict], prod_vis_mode = False):
    ...
    for instruction_number, instruction in enumerate(instructions):
        ...
        # operacje testowe
            ...
            modified_nodes.extend(operation_winning(ls, variant, instruction, prod_vis_mode))
...
def game_over(gp, reason = None):
    ...
    diagram_file_path = gp["DiagramPath"]
    ...
    if diagram_file_path:
        color_gameplay_on_diagram(file_path, diagram_file_path)
...
def get_quest_description(quest_name):
    desc = {
        ...
        "quest00_DragonStory": 'Jesteś głównym bohaterem gry (Main_hero). Twoim celem w tej misji jest zdobycie smoczego jaja pilnowanego przez groźnego smoka. Smoka możesz zabić w walce (pod warunkiem, że uzyskasz odpowiednio dużo siły), otruć lub wypłoszyć z legowiska. Siłę zwiększamy jedząc obiekty o dużej wartości odżywczej, truciznę dostaniemy od przyjaciół lub pozyskamy z trujących roślin.',
        "quest2022-00_DragonStory": 'Jesteś głównym bohaterem gry (Main_hero). Twoim celem w tej misji jest zdobycie smoczego jaja pilnowanego przez groźnego smoka. Smoka możesz zabić w walce (pod warunkiem, że uzyskasz odpowiednio dużo siły), otruć lub wypłoszyć z legowiska. Siłę zwiększamy jedząc obiekty o dużej wartości odżywczej, truciznę dostaniemy od przyjaciół lub pozyskamy z trujących roślin.',
