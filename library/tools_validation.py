    ...
    if "Preconditions" in production:
        for prec in production["Preconditions"]:
            ...
            if 'Cond' not in prec and 'Count' not in prec:
                ...
                # prec['Count'] = re.sub(r'/\*\*/', '/', prec['Count'])  Ale nie na oryginale, na Boga!
                # możemy po prostu wyciąć wszystkie ** ponieważ jeśli multireferencja jest poprawna, to bez gwiazdek też będzie poprawna
                multireference_split = re.sub(r'/\*\*/', '/', prec['Count']).split('/')
...
def production_names_list_builder(jsons_schema_validated: list, production_titles_dict:dict = None, errors:list = None, warnings:list = None):
    ...
    if production_titles_dict is None:
        production_titles_dict = {}
        production_titles_short = {}
    else:
        production_titles_short = {x.split(" / ")[0]: production_titles_dict[x] for x in production_titles_dict}

    for json_instance in jsons_schema_validated:
        for production in json_instance['json']:
            if production['Title'].split(" / ")[0] not in production_titles_short:  # production_titles_dict:
                production_titles_dict[production['Title']] = {"production": production, "file_path": json_instance['file_path']}
                production_titles_short[production['Title'].split(" / ")[0]] = production_titles_dict[production['Title']]
            else:
                if production == production_titles_short[production["Title"].split(" / ")[0]]["production"]:
                    wrn_comm = f'Tytuł produkcji „{production["Title"].split(" / ")[0]}” został już użyty w pliku: \n{production_titles_short[production["Title"].split(" / ")[0]]["file_path"]}.'
                    ...
                    err_comm = f'Tytuł produkcji „{production["Title"].split(" / ")[0]}” został już użyty w pliku: \n{production_titles_short[production["Title"].split(" / ")[0]]["file_path"]}.'
                    err_comm += "\nProdukcje te się różną. Proszę poprawić lub usunąć drugą."
