...
def clean_unfinished_gameplays(root_folder):
    os.makedirs(os.path.join(root_folder, "unfinished"), exist_ok=True)
    os.makedirs(os.path.join(root_folder, "untouched"), exist_ok=True)
    file_names = glob.glob(f"{root_folder}{os.sep}*{os.sep}gameplay*.json")
    for name in file_names:
        print(name)
        gameplay_log = json.load(open(name, encoding="utf8"))
        if "FilePath" in gameplay_log:
            print("          ", gameplay_log["FilePath"])
            print("          ", gameplay_log["Moves"])
            gp_dir = name.rsplit(os.sep, maxsplit=1)[0]
            print("          ", gp_dir)
            new_path = shutil.move(gp_dir, os.path.join(root_folder, "unfinished"))
            print(new_path)
        elif len(gameplay_log["Moves"]) == 0:
            print("          ", gameplay_log["Moves"])
            gp_dir = name.rsplit(os.sep, maxsplit=1)[0]
            print("          ", gp_dir)
            new_path = shutil.move(gp_dir, os.path.join(root_folder, "untouched"))
            print(new_path)


def rename_gameplays(root_folder):

    file_names = glob.glob(f"{root_folder}{os.sep}*{os.sep}gameplay_*.json")
    for name in file_names:
        print(name)
        gameplay_log = json.load(open(name, encoding="utf8"))
        if "QuestName" in gameplay_log:
            print("          ", gameplay_log["QuestName"])
            # print("          ", gameplay_log["Moves"])
            gp_dir = name.rsplit(os.sep, maxsplit=1)[0]
            if gameplay_log["QuestName"] not in gp_dir:
                print("          ", gp_dir)
                new_path = shutil.move(gp_dir, gp_dir + f'__{gameplay_log["QuestName"]}')
                print("          ", new_path)
